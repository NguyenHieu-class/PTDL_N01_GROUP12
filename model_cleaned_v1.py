...
OUTPUT_PATH = "processed_lazada_v1.csv"
LOG_FAIL_PATH = "fail_log_v1.txt"
...
# ===== Tạo prompt yêu cầu trích thông tin =====
...
def append_output(row, extracted):
...
# ===== Ghi log lỗi =====
def log_fail(key, error):
    with open(LOG_FAIL_PATH, "a", encoding="utf-8") as f:
        f.write(f"[key_0 = {key}] Lỗi: {error}\n")
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Đang xử lý"):
            ...
            if not isinstance(extracted, dict):
                log_fail(row["key_0"], extracted)
                continue

            append_output(row, extracted)
